...
Bindwidth
```{r}
ggplot(diamonds, aes(price))+ geom_histogram(color="blue", fill= "skyblue", bins=100)
ggplot(diamonds, aes(price))+ geom_histogram(color="blue", fill= "skyblue", bins=50)
```
...
```{r}
ggplot(diamonds, aes(clarity))+geom_bar()+theme_classic()
```
...
```{r}
ggplot(diamonds, aes(clarity))+geom_bar()+theme_classic()+ xlab(label= "Diamond's \n Clarity") + ylab(label= "Frequency")
```
Note - \n- starts a new line 
...
```{r}
graphDiamonds
ggplot(graphDiamonds, aes(color, price))+ geom_bar(stat="identity")+theme_classic()
```
...
```{r}
ggplot(graphDiamonds, aes(color, price))+ geom_bar(stat="identity")+theme_classic()+ geom_errorbar(aes(ymin= price-se, ymax= price+se))
```
Things I included:
