...
```{r}
class(datFinal$conditionTwo)
datFinal$conditionTwo
```
...
dat$parentReportGender<- relevel(dat$parentReportGender, ref="girl")
dat$parentReportGender
```
...
```{r}
datFinal$parentReportGender<- factor(datFinal$parentReportGender)
levels(datFinal$parentReportGender)<- relevel(datFinal$parentReportGender, ref="F")
```
...
summary(datFinal$anxietyScore)
"we could practice on this, but we will make a new column to use"
datFinal$anxietyScoreTwo<- datFinal$anxietyScore
```
...
datFinal$anxietyScoreTwo<- car::recode(datFinal$anxietyScoreTwo, '1=20; 2=19; 3=18; 4=17; 5=16; 6=15; 7=14; 8=13; 9=12; 10=11; 11=10; 12=9; 13=8; 14=7; 15=6; 16=5; 17=4; 18=3; 19=2; 20=1')
```
...
datFinal %>%  separate(subjNo, into= c("condiiton","subject"),1)

separate(datFinal, subjNo, into= c("condition", "subject"), 1)
```
...
starwars %>% 
  mutate(heightInches= height*.393701) %>% 
  select(height,heightInches)

View(mutate(starwars, heightInches= height*.39701))
```
