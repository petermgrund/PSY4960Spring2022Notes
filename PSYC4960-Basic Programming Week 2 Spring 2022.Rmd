...
```{r}
colnames(diamonds)[6]<- "AMANDA"
colnames(diamonds)
colnames(diamonds)[6]<- "table"
```
...
```{r}
ChickWeight$color<- NA
```
Then you can fill it with whatever information you want. 
```{r}
ChickWeight$color<- "yellow"

ChickWeight$color<- rep(c("yellow","purple"), nrow(ChickWeight)/2)
```
or you can fill the new column in based on other data in the dataframe
```{r}
ChickWeight$day<- ChickWeight$time
```
...
as.numeric(levels(mtcars$hp))[mtcars$hp]
...
numberTwo<- addTwo(number)
numberTwo
number<-data.frame(number)
number$numberTwo<- addTwo(number)
number
```

function to mulitply by 7
```{r}
multiseven<- function(num){
  num * 7
}

multiSeven2<- function(hippo){
  hippo * 7
}
multiseven(8)
multiSeven2(8)
...
```{r}
mult<- function(thing1, thing2){
  thing1 * thing2
}
mult(2,3)
mult(number$number,number$numberTwo)
```
