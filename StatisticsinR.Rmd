...
set.seed(13)
participants<- 1:6000
oppenness<- sample(1:100, length(participants),replace= TRUE)
...
personalityDat<- cbind.data.frame(participants, oppenness, agreeableness, neuroticism, conscientiousness, extraversion)
```

```{r}
personalityDat$participants<- as.factor(personalityDat$participants)
```
...
**mean** 
```{r}
mean(personalityDat$oppenness)
```
**median**
```{r}
median(personalityDat$oppenness)
```
**mode**
```{r}
mode(personalityDat$oppenness) #gives structure of the data
library(modeest)
mfv(personalityDat$oppenness)
```

```{r}
library(ggplot2)
ggplot(personalityDat, aes(oppenness))+geom_histogram(bins=25)
```

#### Variability
**range**
```{r}
range(personalityDat$oppenness)
# range is 99
```

**interquartile range**
```{r}
IQR(personalityDat$oppenness)
ggplot(personalityDat, aes(oppenness))+geom_boxplot()
```
**standard deviation**
```{r}
sd(personalityDat$oppenness)
```
**variance**
```{r}
var(personalityDat$oppenness)
sd(personalityDat$oppenness)^2
```

**summary**
```{r}
summary(personalityDat$oppenness)
```
...
```{r}
personalityDat$opennessScale<-scale(personalityDat$oppenness)
#check
head(personalityDat$opennessScale)
head(personalityDat)
ggplot(personalityDat, aes(oppenness))+geom_histogram()
ggplot(personalityDat, aes(opennessScale))+geom_histogram()
```
...
cor(x, y)
```{r}
cor(personalityDat$conscientiousness, personalityDat$agreeableness)
```
...
cor.test(x,y)
```{r}
cor.test(personalityDat$conscientiousness, personalityDat$agreeableness)
```
...
```{r}
cor.test(personalityDat$conscientiousness, personalityDat$agreeableness, alternative="greater")
...
t.test(variable, mu)
```{r}
t.test(personalityDat$agreeableness, mu = 50)
```
